    ...
    for i in range(0, len(listOfEveryPageUrls)):
        ...
        for hrefclass in hrefsclasses:
            ...
            if len(listOfProductLinks) >= index + 20:
                result_for_20_multiprocess_product=doMultiprocessing(listOfProductLinks[index:index + 20])
                print(result_for_20_multiprocess_product)
                index = index + 20
    return f"Scrapped the details of {len(listOfProductLinks)} products"
    

def doMultiprocessing(listOfProductLinks):
    pool = Pool(processes=3)  
    result=pool.map(scrapProductDetails, listOfProductLinks)
    pool.close()
    pool.join()
    print(result)
    listOfDict1 =result
    # listOfDict1   =list(result)
    result1=  writeInFile(listOfDict1)
    listOfDict1[:] = []
    print(result1)
    return f"Multiprocessed completed for 20 products"
